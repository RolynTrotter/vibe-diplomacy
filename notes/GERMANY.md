# GERMANY — strategy notebook

## Standing plan
- Secure Denmark and Holland early (fleet KIE); keep England calm re: Scandinavia.
- Court Austria against Russia; encourage AUS/ITA vs Russia in the east.
- Avoid early Burgundy war with France — propose BUR DMZ or joint pressure on Russia/England instead.
- Long game: pivot armies toward Russia (Prussia/Silesia) or wherever the weakest neighbor is once Denmark/Holland secured.
- Keep two-front risk (France + Russia) in mind — don't provoke both at once.

## Trust & deals
- AUSTRIA: friendly, confirmed. DEAL: no friction, mutual Russia watch, no territorial overlap (not neighbors).
- ENGLAND: confirmed. DEAL: England takes Norway, Germany takes Denmark/Kiel-area expansion, North Sea uncontested this turn (England's F EDI goes NWG/Norway, not NTH; I make no convoy/fleet play for Norway). Long-term: floated joint England+Germany squeeze on Russia (StP/Baltic) once consolidated — not firm yet, just aligned interest.
- FRANCE: confirmed. DEAL: Burgundy stays quiet/DMZ-in-spirit this turn (no A MUN move on BUR, no French move on BUR either per their msg), France goes for Iberia, I go Denmark/Holland. Calm western front.
- ITALY: unknown — no contact yet
- RUSSIA: unknown, currently 4 centers (likely a report quirk this early) — watch closely. Both England and Austria independently flagged this — common ground for future coordination.
- TURKEY: unknown — no contact yet

## Next phase
- S1901M resolved cleanly, all deals held (Denmark secured, Burgundy quiet, no NTH incursion).
- SUBMITTED F1901M: A RUH-HOL, F DEN H, A KIE H.
- Rationale: grab Holland now that Denmark is safe; F DEN holds to keep the "north stays calm" promise to England legible; A KIE holds as backfill/reserve (RUH is vacating).
- Sent follow-ups: ENGLAND (confirmed deal held, told them F DEN holds/consolidates, floated concrete anti-Russia/StP talk next); FRANCE (confirmed Burgundy DMZ holding, flagged the Holland move isn't a Belgium play, offered to divide Belgium/Channel coast so we don't collide there).
- Watch for: France's response on Belgium (did they claim it or leave it open?), England's Norway/NWG move confirmed, Russia's actual center count and Baltic fleet.
- Next turn: with Holland likely secured, look at Belgium (race vs France) or start pivoting east per the standing plan.
