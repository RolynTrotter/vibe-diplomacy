# ENGLAND — strategy notebook

## Standing plan
- Secure home centers (EDI, LON, LVP) with defensive posture
- Build friendly relations with France and Germany (western neighbors)
- Monitor Russia's expansion (4 centers already — potential threat)
- Expand into Low Countries / North Sea as opportunity allows
- Maintain naval superiority in North Atlantic and English Channel

## Trust & deals
- FRANCE: Channel DMZ confirmed F1901M | mutual commitment to avoid western conflict
- GERMANY: non-aggression pact reaffirmed F1901M | watching their moves
- ITALY: unknown | no deal yet
- RUSSIA: 4 centers, clear threat | need western coalition to contain
- AUSTRIA: unknown | no deal yet
- TURKEY: unknown | no deal yet

## F1901M moves submitted
- F NTH → SKA (expand north toward Scandinavia)
- F ENG → BEL (control Low Countries gateway, honor Channel DMZ)
- A YOR → LON (consolidate south, prepare for future expansion)

## Tactical notes
- Belgium is neutral, touching both France and Holland — good expansion hub
- Skagerrak move pressures Scandinavia, positions against Russian Baltic
- Three fleets + one army allows flexibility for builds
- Watch for Germany's next move — they're in Denmark now, could go multiple directions

## W1901A adjustment (submitted)
- Built F LVP (chose fleet over army for naval superiority goal)
- Now have F SKA, F BEL, F LVP, A LON (3 fleets + 1 army)
- This positions well for spring 1902 expansion across North Sea/Atlantic

## S1902M moves submitted
- F SKA → NWY (secure Norway, block Russia's westward expansion)
- F BEL → HOL (expand south into Low Countries, secure 2nd new center)
- A LON → YOR (consolidate army position for future support)
- F LVP → IRI (defend Irish Sea, maintain western coast presence)

**Orders sealed & pushed.** Expected centers after: EDI, LON, LVP, BEL, NWY, HOL = 6 centers (if all succeed).

## Diplomacy this phase
- Messaged GERMANY: proposed Low Countries coordination + reaffirmed non-aggression
- Messaged FRANCE: suggested informal anti-Russia coalition, maintained Channel DMZ
- Goal: Position as western anchor against Russian expansion (5 centers is dangerous)

## Trust & deals (updated)
- FRANCE: Channel DMZ confirmed | building anti-Russia coalition
- GERMANY: non-aggression pact reaffirmed, coordinating Low Countries strategy
- RUSSIA: Now at 5 centers (threat level elevated) — must contain via coalition
- ITALY: unknown | no contact yet
- AUSTRIA: unknown | no contact yet
- TURKEY: unknown | no contact yet
